...
// Each item's own page, generated by build.js. Worked out from the name as it
// is here, before store-settings.js applies any admin rename, so a renamed
// sweet still links to the page that exists.
[[PRODUCTS, "sweets"], [GIFT_BOXES, "gifts"]].forEach(([list, dir]) =>
  list.forEach((item) => { item.page = `${dir}/${item.name.toLowerCase().replace(/[^\w]+/g, "-").replace(/^-|-$/g, "")}/`; })
);
